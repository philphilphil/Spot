...
func main() {
	board := dragontoothmg.ParseFen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1")
...
func getBoardValueForOneSide(bb *dragontoothmg.Bitboards) int {
	...
	fmt.Println(getPiecePositionalValue(&bb.Pawns, whitePawns))
	panic("s")
...
func getPiecePositionalValue(bb *uint64, values [64]int) int {
	var value int
...
// Get index of pieces starting down left = 1
func getPieceSquareNumbers(bb *uint64) []int {
	...
	for i := 0; i <= 63; i++ {
		if *bb&piecePositionMasks[i] != 0 {
			squareNumbers = append(squareNumbers, i+1)
		}
	}
